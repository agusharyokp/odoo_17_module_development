...
class EstateProperty(models.Model):
    ...
    @api.model
    def _expand_groups(self, values, domain, order):
    ...
    def _onchange_garden(self):
        if self.garden:
            ...
            return {'warning': {
                'title': _("Warning"),
                'message': ('This option will enable garden area (10) and garden orientation (North)')}}
        else :
    ...
    def action_sold(self):
        for record in self:
            if record.state == 'canceled':
                raise UserError("Canceled property to cannot be Sold")
    ...
    def action_cancel(self):
        for record in self:
            if record.state == 'sold':
                raise UserError("Sold property to cannot be Canceled")
